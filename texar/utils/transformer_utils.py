...
def _batching_scheme(batch_size,
    ...
    Args:
        ...
        drop_long_sequences: bool, if True, then sequences longer than
            `max_length` are dropped. This prevents generating batches
            with more than the usual number of tokens, which can cause
    ...
    # highly-compoisite window size and then round down all batch sizes to
    # divisors of that window size, so that a window can always be divided
    # evenly into batches.
