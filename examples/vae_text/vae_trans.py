...
def _main(_):
    ...
    if config.decoder_hparams["type"] == "lstm":
        ...
        decoder_initial_state_size = tf.TensorShape([1, config.emb_hparams["dim"]])
    else:
    ...
    debug_data = data_batch["text_ids"][:, 1:]
    ...
    def _train_epochs(sess, epoch, step, display=10):
        ...
        while True:
            try:
                ...
                if step % display == 0:
                    print('epoch %d, step %d, nll %.4f, klw: %.4f, KL %.4f, rc %.4f, ' \
                           'log_ppl %.4f, ppl %.4f, time elapsed: %.1fs' % \
                          (epoch, step, nll_ / num_sents, opt_vars["kl_weight"], kl_loss_ / num_sents, rc_loss_ / num_sents,
                           nll_ / num_words, np.exp(nll_ / num_words), time.time() - start_time))
    ...
    def _val_epochs(sess, epoch):
        ...
        while True:
            try:
                ...
                print('\nVAL: epoch %d, nll %.4f, KL %.4f, rc %.4f, log_ppl %.4f, ppl %.4f\n' %
                      (epoch, nll_ / num_sents, kl_loss_ / num_sents, rc_loss_ / num_sents, 
    ...
    def _test_epochs(sess, epoch):
        ...
        while True:
            try:
                ...
                print('\nTEST: epoch %d, nll %.4f, KL %.4f, rc %.4f, log_ppl %.4f, ppl %.4f\n' %
                      (epoch, nll_ / num_sents, kl_loss_ / num_sents, rc_loss_ / num_sents,
                       nll_ / num_words, np.exp(nll_ / num_words)))
                break
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(config.num_epochs):
            ...
            if val_nll < opt_vars['best_valid_nll']:
                ...
                if opt_vars['steps_not_improved'] == config.lr_decay_hparams["threshold"]:
