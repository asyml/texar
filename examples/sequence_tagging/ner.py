...
char_embedder = tx.modules.WordEmbedder(vocab_size=char_size, init_value=char_vecs)
...
outputs = tf.concat([outputs, char_outputs], axis=2)

rnn_shape = tf.shape(outputs)
output_dim = 2 * config.hidden_size + config.conv['filters']
