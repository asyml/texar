...
def _main(_):
    ...
    encoder_output = tf.placeholder(tf.float32, [None, 1, config.hidden_size])
    ...
    print('overall params size:{}'.format(sum(params_sizes)),
        file=training_log)
    training_log.flush()

    #l1_loss = sum([tf.nn.l2_loss(t) for t in tf.trainable_variables])
    ...
    def _run_epoch(sess, data_iter, mode_string):
        if mode_string == 'train':
            cur_batch_size = config.train_batch_size
        ...
        for step, (x, y) in enumerate(data_iter):
            ...
            if step == 0:
                feed_dict[encoder_output] = np.zeros((cur_batch_size,
                    1, config.emb['dim']))
            else:
            ...
            # TODO(haoran):this is a hard-coded hyperparams to check for lr update
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(config.num_epochs):
            ...
            train_data_iter = ptb_iterator(
                data["train_text_id"], config.train_batch_size, num_steps)
