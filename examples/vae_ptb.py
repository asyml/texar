...
def _main(_):
    ...
    output_embedding = embedder(data_batch["text_ids"])

    if keep_prob < 1:
        output_embedding = tf.nn.dropout(
            output_embedding, tx.utils.switch_dropout(keep_prob))
    ...
    def _train_epochs(sess, epoch, step, display=10):
        ...
        while True:
            try:
                ...
                new_kl_rate = kl_anneal_function(step, anneal_hparams["k"],
                                                 anneal_hparams["x0"])
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(num_epochs):
            step, new_kl_rate = _train_epochs(sess, epoch, step, display=200)
