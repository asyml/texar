...
arg_parser.add_argument('--decay_k', type=float, default=4000.)
...
log_dir = args.task + '_training_log' + '_ss_decayK' + str(args.decay_k) + '/'
os.system('mkdir ' + log_dir)
valid_test_log_file = open(log_dir + 'valid_test_log.txt', 'w')


def inverse_sigmoid(i):
    return args.decay_k / (args.decay_k + math.exp(i / args.decay_k))
...
def get_train_op(data_batch, decoder_cell, embedder,
    ...
    decoder_hparams['helper_train']['type'] = 'ScheduledEmbeddingTrainingHelper'
    ...
    sampling_proba = tf.placeholder(
        dtype=tf.float32, shape=[], name='sampling_proba')
    ...
    return sampling_proba, decoder, tx.core.get_train_op(
...
def build_model(data_batch, batch_size, source_vocab_size, target_vocab_size,
    ...
    sampling_proba, training_decoder, train_op = get_train_op(
    ...
    return sampling_proba, train_op, infer_outputs
...
def main():
    ...
    sampling_proba, train_op, infer_outputs = build_model(
        data_batch, batch_size, source_vocab_size, target_vocab_size,
        target_bos_token_id, target_eos_token_id)

    def train_epoch(sess, epoch, training_batches_counter):
            ...
            try:
                ...
                print(counter,
                      sess.run(train_op, feed_dict={
                          ...
                          sampling_proba: 1. - inverse_sigmoid(training_batches_counter)}),
                      'sampling_proba:', 1. - inverse_sigmoid(training_batches_counter),
                      file=log_file)
                log_file.flush()
    ...
    with tf.Session() as sess:
        ...
        for i in range(args.num_epochs):
            train_epoch(sess, i, training_batches_counter)
